...
```{r}
ggplot(cdi_df, aes(x = poverty, y = crime_rate)) + geom_point(alpha = .5) + geom_smooth(method = "lm", se = FALSE)

reg_poverty = lm(cdi_df$crime_rate ~ cdi_df$poverty) 
summary(reg_poverty)
fitted_value =  reg_poverty$fitted.values
reg_poverty %>% broom::tidy()

plot(cdi_df$poverty, cdi_df$crime_rate)
abline(reg_poverty, lwd = 2, col = 2)
```
...
```{r}
ggplot(cdi_df, aes(x = beds, y = crime_rate)) + geom_point(alpha = .5) + geom_smooth(method = "lm", se = FALSE)

reg_beds = lm(cdi_df$crime_rate ~ cdi_df$beds) 
summary(reg_beds)
reg_beds %>% broom::tidy()
```
...
```{r}
ggplot(cdi_df, aes(x = docs, y = crime_rate)) + geom_point(alpha = .5) + geom_smooth(method = "lm", se = FALSE)

reg_docs = lm(cdi_df$crime_rate ~ cdi_df$docs) 
summary(reg_docs)
reg_docs %>% broom::tidy()
```
...
```{r}
ggplot(cdi_df, aes(x = hsgrad, y = crime_rate)) + geom_point(alpha = .5) + geom_smooth(method = "lm", se = FALSE)

reg_docs = lm(cdi_df$crime_rate ~ cdi_df$hsgrad) 
summary(reg_docs)
reg_docs %>% broom::tidy()
```

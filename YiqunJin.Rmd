...
ggplot(cdi_model, aes(x = poverty, y = hsgrad, color = region)) + 
  geom_point() +
...
ggplot(cdi_model, aes(x = poverty, y = unemp, color = region)) + 
  geom_point() +
  geom_smooth(method = "lm", se = F, aes(group = region, color = region)) 
...
```{r}
poverty_compare = cdi_model %>% 
  mutate(national_poverty = if_else(poverty > 13.5, "higher", "lower"))

ggplot(poverty_compare, aes(x = pcincome, y = crime_rate_1000, color = national_poverty, alpha = .5)) + 
  geom_point() +
  geom_smooth(method = "lm", se = F, aes(group = national_poverty, color = national_poverty)) 

# fit model with interaction
interact7 = lm(crime_rate_1000 ~ pcincome*national_poverty, data = poverty_compare)
summary(interact7)
```
...
```{r}
bagrad_compare = cdi_model %>% 
  mutate(national_bagrad = if_else(bagrad > 20.8, "higher", "lower"))

ggplot(bagrad_compare, aes(x = pcincome, y = crime_rate_1000, color = national_bagrad, alpha = .5)) + 
  geom_point() +

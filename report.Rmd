...
Transform vairables in order to extract interpretable information from correlated predictors.

2. Exploratory analysis:

* Calculate the pairwise correlations between variables

* List all correlations between the crime rate (our interest) and other variables

3. Training/Testing set split:

Randomly split the dataset into training (90%) and testing sets (10%) to avoid overfitting. 

4. Remove outliers and high leverage points

Use percentile to detect potential outliers and high leverage points. Remove rows containing the smallest and largest 0.2% for each variable given the size of the dataset.
...
* Build model using selected variables and interaction terms (p-value < 0.2)

* Diagnose and transform the models

6. Cross validation

Use 5-fold cross validation on each model to estimate model performances.

7. Model assessment

Assess the models and choose a final model used for future prediction. Compare the models based on three criteria: the R-square values, the root mean square error (RMSE), and the root mean square prediction error (RMSPE).  
...
We added interaction terms to the selected variables, resulting in the following three models:
...
Our first model uses only variables we selected: $$ \begin{aligned}
                  crime\_rate\_1000 &=\beta_0 +\beta_1 pop + \beta_2 pop18 + \beta_3 bagrad +
                  \beta_4 poverty + \beta_5 unemp \\&+ \beta_6 pcincome + \beta_7 pcincome*pop + \beta_8 region + \beta_9 beds\_rate\_1000 + \beta_{10}density
                  \end{aligned}$$
...
Our second model: $$\begin{aligned} crime\_rate\_1000 &= pop + pop18 + 
                  poverty + unemp + pcincome \\ &+ pcincome*pop + region +
...
Our third model: $$\begin{aligned} crime\_rate\_1000 &= pop + pop18 + bagrad +
                  poverty + unemp + pcincome \\&+ pcincome*pop + region +
...
Here is the summary table of for our three models:

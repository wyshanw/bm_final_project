...
In addition, we drew boxcox plots to see if each model needed transformation. The peak of all three boxcox plots fell between 0.5 and 1 (see .Rmd for details). As such, we tried $\sqrt{y}$ transformation for each model. Since the residuals were more unevenly distributed in all three transformed models, we kept the three untransformed models (Table 3).

## Model Assessment

We performed cross validation on each model and got the RMSE average and R-square values. We further assessed the models in the testing set by RMSPE (Table 4).
